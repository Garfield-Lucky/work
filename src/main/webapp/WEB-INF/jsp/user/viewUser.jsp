...
<form name="form" id="form" method="post" class="layui-form" action="${pageContext.request.contextPath }/user/editUser" onsubmit="return ajaxSave(this);">
    <table class="layui-table" width="100%">
        ...
        <tr>
            <td class="left" >账号:</td>
            <td class="right">
                <input type="text" id="userName" name="userName" VALUE="${user.userName}" class="layui-input" disabled />
                <input type="hidden" id="id" name="id" VALUE="${user.id}" class="layui-input" disabled />
            </td>
        </tr>
        <tr>
            <td class="left" >密码:</td>
            <td class="right">
                <input type="text" id="password" name="password" VALUE="${user.password}" class="layui-input" disabled />
            </td>
        </tr>
        <tr>
            <td class="left" >真实姓名:</td>
            <td class="right">
                <input type="text" id="trueName" name="trueName" VALUE="${user.trueName}" class="layui-input" disabled />
            </td>
        </tr>
        <tr>
            <td class="left"  >年龄:</td>
            <td class="right">
                <input type="text" id="age" name="age" VALUE="${user.age}" class="layui-input" disabled />
            </td>
        </tr>
        <tr>
            <td class="left"  >性别:</td>
            <td class="right">
                <input type="radio" name="sex" value="男" title="男" disabled <c:if test="${user.sex =='男'}"> checked </c:if>>
                <input type="radio" name="sex" value="女" title="女" disabled <c:if test="${user.sex =='女'}"> checked </c:if>>
            </td>
        </tr>
        <tr>
            <td class="left"  >电话:</td>
            <td class="right">
                <input type="text" id="tel" name="tel" VALUE="${user.tel}" class="layui-input" disabled />
            </td>
        </tr>
        <tr>
            <td class="left"  >微信:</td>
            <td class="right">
                <input type="text" id="wx" name="wx" VALUE="${user.wx}" class="layui-input" disabled />
            </td>
        </tr>
        <tr>
            <td class="left"  >状态:</td>
            <td class="right">
                <input type="checkbox" id="status" name="status" lay-skin="switch" disabled lay-text="开启|禁用" lay-filter="status" VALUE="${user.status}" <c:if test="${user.status ==0}"> checked </c:if>/>
            </td>
        </tr>
